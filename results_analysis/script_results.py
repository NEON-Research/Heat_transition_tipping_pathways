...
def generate_all(df, outdir):
    """Generate the full plot set into `outdir`. Each plot is isolated: a plot that needs a
    scenario/ownership not present in this run is skipped with a warning, so a single-scenario
    run doesn't crash the multi-scenario figures."""
    import os
    os.makedirs(outdir, exist_ok=True)
    scenarios = set(df['scenario_name'].unique()) if 'scenario_name' in df.columns else set()
    ...
    def step(label, fn):
        try:
            ...
            print(f"[analyze] skipped '{label}': {type(e).__name__}: {e}")

    step("stacked percentage plots", lambda: create_stacked_percentage_plots(df, output_dir=f"{outdir}/stacked_percentage_plots"))
    step("combined ownership plot", lambda: create_combined_plot(df, output_file=f"{outdir}/combined_ownership_plot.png"))
    step("scenario comparison TOTAL", lambda: create_total_ownership_scenario_comparison(df, output_file=f"{outdir}/scenario_comparison_TOTAL.png"))
    sel = [s for s in ["policy_driven_dh_strategy", "policy_driven_sha_strategy",
                       "actor_allignment_strategy", "dh_policy_based_connection_obligation"] if s in scenarios]
    if len(sel) >= 2:
    ...
    print(f"[analyze] done -> {outdir}")


if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="Plot a simulation_results CSV into a set of figures.")
    ap.add_argument("--input", default="results/simulation_results.csv", help="path to the results CSV")
    ap.add_argument("--outdir", default="plots", help="directory to write figures into")
    a = ap.parse_args()
    df = load_and_prepare_results(a.input)
    generate_all(df, a.outdir)