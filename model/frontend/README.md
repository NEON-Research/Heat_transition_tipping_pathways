...
> **⚠ Depends on the deprecated JS engine.** This dashboard imports the JavaScript engine from
> `../../deprecated/engine/` (the retired reference oracle), NOT the Java model that is now the
> source of truth. It still runs, but it does not reflect Java-only changes. To make it a true
> front end on the Java model it needs rebuilding — e.g. run the Java engine and load its CSV, or
> expose the Java engine over a small bridge. Tracked as future work.
...
- Runs `Simulation` (../../deprecated/engine/src/modelFull.js) and draws:
...
import csv, json
rows=[]
for i,r in enumerate(csv.DictReader(open("model/data-export/out/limburg_dwellings.csv"))):
    if i%100: continue
...
json.dump({"fields":["buurt","dtype","label","area","space","dhw","insulB","insulC","koop","huur","corp"],
           "rows":rows}, open("model/frontend/limburg_sample.json","w"))
PY
```
