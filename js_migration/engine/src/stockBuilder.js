...
function blockAvgLabel(hh) {                               // J_HousingBlock.setDefaultEnergyLabel
  return numLabel(Math.round(hh.reduce((a, d) => a + labelNum(d.energyLabel), 0) / hh.length));
}
...
function assignInitialHeating(rng, nbh, perc) {
  ...
  if (socialSize > 0) {
    ...
    nbh.socialBlock = { buurt: nbh.buurt, households: nbh.socialHH, currentType: type,
  ...
  for (const d of nonSHB) { if (setNGBl >= reqNGBl) break; if (d.archetype === 'APARTMENT') put(d); }
  ...
  if (hoaHH.length) { const _ylrH = blockYearLastRenovation(rng, hoaHH);
    nbh.hoaBlock = { buurt: nbh.buurt, households: hoaHH, currentType: 'NATURAL_GAS_BLOCK',
...
export function buildAgents(rows, F, { rng, archetypeRows, nbhData, everyNth = 1 }) {
  ...
  for (const f of rows) {
    ...
    const d = { id: id++, ownership: own, currentType: 'NATURAL_GAS_BOILER', age: rng.int(0, 12),
      ...
      hasLowTemp: label === 'a' || label === 'b', hasDistrictHeatingGrid: !!(nb && nb.hasDHgrid),
  ...
  return { homeowners, landlords, socialBlocks, hoaBlocks, archIndex };
}
