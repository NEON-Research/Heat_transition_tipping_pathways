...
`js_migration/engine-java/` (see its README). During exploration a **JavaScript engine** was
also built (`js_migration/engine/`); it is retained as a *reference oracle* — a runnable
...
1. **Engine core — DONE.** Java: `js_migration/engine-java/` (seeded RNG, pure decision
   ...
   twin (`js_migration/engine/`, tested green here) serves as the reference oracle.
2. **Real data — DONE (export + loaders).** `js_migration/data-export/` extracts the real
...
See `js_migration/MODEL_REFERENCE.md` for the per-class/formula reference and
`js_migration/tests/README.md` for how to run the tests.
