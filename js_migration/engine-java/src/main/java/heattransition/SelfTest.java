...
public final class SelfTest {
    ...
    public static void main(String[] args) {
        ...
        ok(rows.size() == 27, "27 years of results");
        int finalTotal = 0;
        for (int v : rows.get(rows.size() - 1).stock.values()) finalTotal += v;
        ok(finalTotal == 500, "stock conserved (=500)");

        // multi-agent smoke test (synthetic stock -> blocks/landlords/homeowners, stock conserved)
        try {
            ...
            Vesta vesta = Vesta.load("../data-export/out/dwellings_demand_insulation.json");
            FullSimulation fs = new FullSimulation(2024, 2050, new Rng(1), new Scenario(), false, 10,
                    ho, ll, java.util.List.of(blk), java.util.List.of(), vesta);
            java.util.List<FullSimulation.YearRow> fullRows = fs.run();
